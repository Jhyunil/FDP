                                ...
                                uint32_t buf_len, uint64_t off)  
{
    ...
    // 2. 요청된 이벤트 타입 확인 (Host vs Controller)
    ...
    if (ebuf->nelems == 0) {
        elog = g_malloc0(sizeof(NvmeFdpEventsLog));
        return dma_read_prp(n, (uint8_t *)elog + off,
                            MIN(buf_len, sizeof(NvmeFdpEventsLog)),
                            le64_to_cpu(cmd->dptr.prp1),
                            le64_to_cpu(cmd->dptr.prp2));
    }
    // 3. 로그 페이지 메모리 할당 (헤더 크기 + 이벤트 개수 * 이벤트 크기)
    uint32_t log_size = sizeof(NvmeFdpEventsLog) + (ebuf->nelems * sizeof(NvmeFdpEvent));
    elog = g_malloc0(log_size);

    // 4. 헤더 채우기
    elog->num_events = cpu_to_le32(ebuf->nelems);

    // 5. 이벤트 리스트 복사 (Flattening)
    event = (NvmeFdpEvent *)(elog + 1);

    for (unsigned int i = 0; i < NVME_FDP_MAX_EVENTS; i++) {
        // femu_log("nvme_fdp_events i: %d\n", i);
        unsigned int idx = (ebuf->start + i) % NVME_FDP_MAX_EVENTS;
        event[i] = ebuf->events[idx];
    }

    // 6. 전송 길이 계산
    trans_len = (buf_len < log_size) ? buf_len : log_size;

    // 7. 호스트 메모리로 전송 (DMA)
    return dma_read_prp(n, (uint8_t *)elog + off, trans_len, cmd->dptr.prp1, cmd->dptr.prp2);
} 																			
