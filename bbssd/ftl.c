...
Heaptype* create_heap(int pgs_per_ru)
{
    Heaptype* h_ptr = (Heaptype*)malloc(sizeof(Heaptype));
    h_ptr->heap = (uint64_t*)malloc(sizeof(uint64_t) * (pgs_per_ru+1));
...
void insert_minheap(Heaptype* h, uint64_t item)
{
	int i;
	i = ++(h->heap_size);
...
uint64_t delete_minheap(Heaptype* h)
{
	if(!h->heap_size) {
		return -1;
	}
...
Heaptype* vpg_list;    // min heap of valid page numbers

static void *ftl_thread(void *arg);

static inline bool should_gc_high(struct ssd *ssd, uint16_t rg)
{
    // femu_log("Check GC High: free_ru_cnt=%d, thres=%d\n", 
        // ssd->rums[rg].free_ru_cnt, ssd->sp.gc_thres_rus_high);
		...
		uint16_t ruhid, bool for_gc) 
{
	...
	/*****************/
    struct ruh* ruh = &ssd->ruhtbl[ruhid];      // 1. 호스트 핸들 구조체 가져오기
    struct ru_mgmt* rum = &ssd->rums[rgid];     // 2. 해당 RG 관리자 가져오기
    int cur_ruid; 
    struct ru* cur_ru;
    ...
    // 4. 현재 RU 구조체 가져오기
    cur_ru = &rum->rus[cur_ruid];

    // 5. Write Pointer 증가 (Striping: Channel -> LUN -> Page -> Block)
    cur_ru->wp.ch++;

    if (cur_ru->wp.ch == (rgid+1) * RG_DEGREE / ssd->sp.luns_per_ch) { // ssd->sp.nchs
        cur_ru->wp.ch = rgid * RG_DEGREE / ssd->sp.luns_per_ch;
        // cur_ru->wp.ch = 0;
        cur_ru->wp.lun++;

        
        if (cur_ru->wp.lun == ssd->sp.luns_per_ch) {
            cur_ru->wp.lun = rgid * RG_DEGREE % ssd->sp.luns_per_ch; 
            // cur_ru->wp.lun = 0; 
            cur_ru->wp.pg++;

            // if (cur_ru->wp.pg == ssd->sp.pgs_per_blk) {
            //     cur_ru->wp.pg = 0;
            //     cur_ru->wp.blk++;
            // }
            if (cur_ru->wp.pg == ssd->sp.pgs_per_blk) {
                /* need to allocate a new RU */
                int new_ruid = get_next_free_ruid(ssd, rum);
                if (new_ruid < 0) {
                    ftl_err("No free reclaim units left in [%s] !!!!\n", ssd->ssdname);
                    return;
                }
                ...
                if (for_gc) {
                    rum->ii_gc_ruid = new_ruid;
                } else {
                    ruh->cur_ruids[rgid] = new_ruid;
                }
                rum->rus[new_ruid].ruhid = ruhid;

                if (cur_ru->ipc == 0) {
                    // 모든 data가 유효하면 -> Full List로 보관
                    QTAILQ_INSERT_TAIL(&rum->full_ru_list, cur_ru, entry);
                    rum->full_ru_cnt++;
                } else {
                    // invalid page가 하나라도 있으면 -> Victim Queue로 보내서 바로 재활용 준비 [cite: 713]
                    pqueue_insert(rum->victim_ru_pq, cur_ru);
                    rum->victim_ru_cnt++;
                }
            }
        }
    }
    // femu_log("rgid : %d , ruid : %d, chn : %d, lun: %d , pl: %d, blk: %d, pg: %d \n", 
        // rgid, cur_ruid, cur_ru->wp.ch, cur_ru->wp.lun, cur_ru->wp.pl, cur_ru->wp.blk, cur_ru->wp.pg);
}																					
		...
		uint16_t ruhid, bool for_gc) 
{
	...
	**************/
    ...
    // 5. ppa 복사
    int ch = cur_ru->wp.ch;
    int lun = cur_ru->wp.lun;
    int pl = cur_ru->wp.pl;
    int blk = cur_ru->wp.blk;
    int pg = cur_ru->wp.pg;
...
void ssd_init(FemuCtrl *n)
{
    ...
    vpg_list = create_heap(spp->pgs_per_ru);
    init_heap(vpg_list);
}
    ...
    for (int pg = 0; pg < spp->pgs_per_blk; pg++) {
        ...
        if (pg_iter->status == PG_VALID) {
            ...
            cnt++;
          	struct ru_mgmt *rum = &ssd->rums[rgid];
            rum->total_valid_cnt++;
            insert_minheap(vpg_list, get_rmap_ent(ssd, ppa));
        }
    }
...
static int do_gc(struct ssd *ssd, uint16_t rgid, bool force, NvmeRequest *req)
{
    ...
    // For Event Logging, initialize meta data
    rum->total_valid_cnt = 0;               // 결과: NLBAM / setors_per_page (Page 단위)
    uint64_t prev_lpn = INVALID_LPN-1;
    uint64_t max_run_len = 0;
    uint64_t max_run_start_lpn = 0;
    ...
    while(vpg_list->heap_size) {
        uint64_t curr_lpn = delete_minheap(vpg_list);
        
        if (curr_lpn == prev_lpn + 1) {                    // 연속성 검사
            curr_run_len++;
        } else {
            if (curr_run_len > max_run_len) {          // 연속이 끊김 -> 최대값 갱신 확인
                max_run_len = curr_run_len;
                max_run_start_lpn = curr_run_start;
            }
            curr_run_start = curr_lpn;                      // 새로운 구간 시작
            curr_run_len = 1;
        }
        prev_lpn = curr_lpn;
    }
    
    if (curr_run_len > max_run_len) {
        max_run_len = curr_run_len;
        max_run_start_lpn = curr_run_start;
    }
	...
	if (ruh->ruht == NVME_RUHT_INITIALLY_ISOLATED && log_event(ruh, FDP_EVT_MEDIA_REALLOC)) {
        ...
        if (e) {
            e->type = FDP_EVT_MEDIA_REALLOC; // 0x80
            e->flags = 0x07; // PIV(1) | NSIDV(1) | LV(1) -> 0000 0111
            e->rgid = rgid;
            e->ruhid = ruhid;
            e->nsid = req->ns->id;
            e->pid = rum->total_valid_cnt ? ((uint16_t)max_run_start_lpn * spp->secs_per_pg) : 0;
            e->timestamp = rum->total_valid_cnt * spp->secs_per_pg; 
        }
	}
...
static uint64_t ssd_write(struct ssd *ssd, NvmeRequest *req)
{
     ...
     *******************************/
    // femu_log("len : %d\n", len);
    NvmeCmd *rw = (NvmeCmd*)&req->cmd;				
    uint8_t dtype = (rw->cdw12 >> 20) & 0xf;
	uint16_t pid = (rw->cdw13 & 0xffff0000) >> 16;

    // if(!pid) femu_log("Bad parsing\n");
    // femu_log("Bad parsing\n");
    // exit(0);

	uint16_t rgif = (uint16_t)endgrp->fdp.rgif;
	uint16_t rgid = pid >> (16 - rgif);
	uint16_t ph = pid & 0x3f;
    // uint16_t rgid = pid & ((1 << rgif) - 1);
    // uint16_t ph = pid >> 10;
    ...
    // femu_log("Check GC in ssd_write for rgid=%d\n", rgid);
