...
NvmeFdpEvent *nvme_fdp_alloc_event(FemuCtrl *n, NvmeFdpEventBuffer *ebuf)
{
	...
	if (is_full)
        ...
        // femu_log("Event logged: total events = %d\n", ebuf->nelems + 1);
		ebuf->nelems++;
    }
...
# 사용법:
#   ./nvme_write_gc.sh [/dev/nvme0n1] [PASS_COUNT] [DATA_MB]
#
# 예:
#   ./nvme_write_gc.sh /dev/nvme0n1 1        # 1 pass, 1MB 단위 write
#   ./nvme_write_gc.sh /dev/nvme0n1 3 4      # 3 pass, 4MB 단위 write
#
# 기본값:
#   DEV=/dev/nvme0n1
#   PASS_COUNT=1
#   DATA_MB=1  (1MB per write)

DEV=${1:-/dev/nvme0n1}
PASS_COUNT=${2:-1}
DATA_MB=${3:-1}

# FDP Data Placement Directive
DIR_TYPE=0x02          # NVME_DIRECTIVE_DATA_PLACEMENT
DIR_SPEC=0x8001        # 네가 테스트에 쓰는 값

SECTOR_SIZE=512
DATA_SIZE=$((DATA_MB * 1024 * 1024))               # bytes
SECTORS_PER_IO=$((DATA_SIZE / SECTOR_SIZE))        # 논리 블록 수 (= start-block 단위 증가량)

echo "[*] Target device : $DEV"
echo "[*] Pass count    : $PASS_COUNT"
echo "[*] Data size     : ${DATA_SIZE}B (${DATA_MB} MB, ${SECTORS_PER_IO} sectors per IO)"
echo "[*] Directive     : dir-type=${DIR_TYPE}, dir-spec=${DIR_SPEC}"

# ns 크기 얻기 (LBA 수)
NSZE=$(nvme id-ns "$DEV" | awk '/^nsze/ {print $3}')
if [ -z "$NSZE" ]; then
    echo "[!] Failed to get nsze from nvme id-ns"
    exit 1
fi

# start-block 기준 최대 시작 위치 (경계 넘지 않도록)
MAX_LBA=$((NSZE - SECTORS_PER_IO))

echo "[*] Namespace size: ${NSZE} LBAs"
echo "[*] Max start-block for ${DATA_MB}MB IO: ${MAX_LBA}"

# 실제 write를 수행하는 함수: 여기서 y/엔터 자동 입력
do_nvme_write() {
    local start_block=$1

    # nvme가 확인 프롬프트를 띄우면 yes가 y⏎를 계속 보내줌
    yes | nvme write "$DEV" \
...
for pass in $(seq 1 "$PASS_COUNT"); do
    echo
    echo "[*] ===== PASS ${pass}/${PASS_COUNT} : Sequential fill (${DATA_MB}MB 단위) ====="

    lba=0
    while [ "$lba" -le "$MAX_LBA" ]; do
        do_nvme_write "$lba"

        # 진행 상황 출력
        if (( lba % (SECTORS_PER_IO * 256) == 0 )); then
            echo "[*] PASS ${pass}: start-block=${lba}/${MAX_LBA}"
        fi

        lba=$((lba + SECTORS_PER_IO))
    done

    echo "[*] PASS ${pass}: sequential fill done"

    echo "[*] ===== PASS ${pass} : Random overwrite phase (GC 유도) ====="
    # 전체 용량의 1/4 정도를 랜덤 overwrite
    RANDOM_WRITES=$((NSZE)) # / (SECTORS_PER_IO * 4)))
    if [ "$RANDOM_WRITES" -lt 1 ]; then
        RANDOM_WRITES=1
    fi

    for i in $(seq 1 "$RANDOM_WRITES"); do
        # 랜덤 start-block (DATA_SIZE 단위 정렬)
        rand_lba=$(( (RANDOM * RANDOM) % MAX_LBA ))
        rand_lba=$(( (rand_lba / SECTORS_PER_IO) * SECTORS_PER_IO ))

        do_nvme_write "$rand_lba"

        if (( i % 256 == 0 )); then
            echo "[*] PASS ${pass}: random overwrite ${i}/${RANDOM_WRITES}"
        fi
    done

    echo "[*] PASS ${pass}: random overwrite done (GC 발생 가능 구간)"
done

echo
echo "[*] All passes done. Check FDP 이벤트 로그"
*/